...
Figure~\ref{fig:closed-loop-cost} compares the policies using realized outcomes from the high-fidelity simulator. With a 10-s MILP limit, scenario rolling-horizon control reduced the mean objective value from 8364 to 8319 relative to scenario block planning and increased fill rate from 0.346 to 0.358. It also reduced mean WIP from 30.1 to 29.2. Relative to the setup-aware heuristic, the proposed policy reduced the objective value by 17.4\%, mean WIP by 74.8\%, and cleaning time by 69.8\%. All scenario rolling-horizon solves returned an accepted feasible status. Because the paired intervals against deterministic and block planning included zero, these operational differences should be interpreted as pilot evidence rather than conclusive statistical superiority.
